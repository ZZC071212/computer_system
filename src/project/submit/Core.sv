...
wire commit_exception = except_info_ex4.except;
    ...
    EXEMEM EXEMEM_reg;
    MEMWB MEMWB_reg;
    wire csr_satp_wb = MEMWB_reg.valid && MEMWB_reg.we_csr && (MEMWB_reg.csr_addr == CsrPack::SATP);
...
always_ff @(posedge clk or posedge rst) begin
    if (rst) begin
        if_state <= IF_IF1;
    end else if (switch_mode || csr_satp_wb) begin
        if_state <= IF_IF1;
    end else begin
        if_state <= if_state_n;
    end
...
always_ff @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        if (flush || switch_mode || csr_satp_wb) begin
            IFID_reg.valid <= 1'b0;
        end
        if (i_req_fire) begin
            if_req_pc          <= pc;
        ...
        if (i_rep_fire) begin
            if_drop_reply <= 1'b0;
        end else if ((flush || switch_mode || csr_satp_wb) && wait_for_i_rep) begin
            if_drop_reply <= 1'b1;
        end
    end
end
...
always_ff @(posedge clk or posedge rst) begin
    if (rst || flush || switch_mode || csr_satp_wb) begin
        IFID_reg.valid <= 1'b0;
        IFID_reg.pc    <= 64'h0;
        IFID_reg.inst  <= 32'h0;
        IFID_reg.pc_4  <= '0;
...
always_ff @(posedge clk or posedge rst) begin
   ...
   IDExceptExamine except_examine (
    ...
    RegFile regfile (
        .clk(clk),
        .rst(rst),
        .we(MEMWB_reg.we_reg && MEMWB_reg.valid && !commit_exception),
        .read_addr_1(rs1_id),
        .read_addr_2(rs2_id),
...
CSRModule u_csr_module(
    ...
    .csr_we_wb( MEMWB_reg.we_csr && !commit_exception),
    ...
    assign cosim_switch_mode = switch_mode | csr_satp_wb;
...
always_ff @(posedge clk or posedge rst) begin
        if (rst || flush ||stall |switch_mode||csr_satp_wb||csr_raw_hazard) begin
...
always_comb begin
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst || switch_mode || csr_satp_wb) begin
    ...
    assign cosim_core_info.rd_we     = {63'b0, MEMWB_reg.we_reg && !commit_exception};
