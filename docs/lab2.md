...
同时与总线交互的状态机可以不需要大改，但是由于加入了cache，流水线不再像之前一样需要几个周期才能取出一个指令执行，也不需要一直访存和取指令，所以IDLE状态时有可能发生 icache miss 或者 dcache miss ，不再是直接进入 IF1 状态，如遇到了dcache miss，则应该进入MEM1状态**（当然不同同学实现不同，更多的应该根据自己的实际情况调试来看）**，遇到了icache miss，才进入IF1状态，其余命中cache的时候应该保持IDLE不变，更多的改变则要根据同学们自己的状态机修改~
