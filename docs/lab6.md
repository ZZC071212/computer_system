...
### 基础地址转换功能的实现
大家可以先实现基础的 MMU 地址转换的功能，成功运行起 lab3 的 kernel之后，再进一步在基础上实现缺页异常。本节将带领你实现基础的地址转换。
...
首先，MMU 先把 mem_ift1 和 mem_ift2 断开，自己做地址转换。因为断开之后，mem_ift1 收不到内存传回的握手信号，所以他就一直保持着 `request_valid` 不变。 等到地址转换完成，MMU拿到物理地址了，再把 mem_ift1 和 mem_ift2 连接起来，将 mem_ift1 即核内的访存地址修改为物理地址。内存传回的握手型号核内可以收到，就会做两次握手的状态转移。
