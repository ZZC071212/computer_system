...
* Linux OS in System II Lab6

## 3. 背景知识

### 3.0 前言

在[系统二实验六](https://zju-sys.pages.zjusct.io/sys2/sys2-fa22/)中，我们赋予了 OS 对多个线程调度以及并发执行的能力，由于目前这些线程都是内核线程，因此他们可以共享运行空间，即运行不同线程对空间的修改是相互可见的。但是如果我们需要线程相互**隔离**，以及在多线程的情况下更加**高效**的使用内存，我们必须引入 `虚拟内存`这个概念。
...
* 此次实验基于[系统二实验六]([实验指导六 - 浙江大学22年秋冬系统贯通二实验 (zjusct.io)](https://zju-sys.pages.zjusct.io/sys2/sys2-fa22/lab6/))中同学所实现的代码进行。
* 需要修改 `defs.h`, 在 `defs.h` `添加` 如下内容：
  ...
  ```
* 从 [`repo`](https://gitee.com/Parfaity/sys3lab-2023-stu) 同步以下代码：`vmlinux.lds` 。并按照以下步骤将这些文件正确放置。
...
将 0x80000000 开始的 1GB 区域映射，将其映射至高地址 ( PA + PV2VA_OFFSET == VA )。如下图所示：
...
本次实验中我们将不再使用原先实验中在代码段后申请栈的方法。我们在`vmlinux.lds`中分配了栈空间，你需要参照给出的`head.S`修改你的实现。
...
>
> Hint 2: 在 set satp 前，我们只可以使用**物理地址**来打断点。需要注意的是GDB加载的符号均为虚拟地址，因此物理地址部分将不会被加载符号。你的代码将被加载到物理地址PHY_START + OPENSBI_SIZE处，调试时可以在这里打断点。
>
...
- 你可以修改 proc.c 中输出的消息来验证实验是否成功，一种参考实现如下：
