...
在 RISC-V 中，hart（hardware thread）是指一个硬件线程，基本可以认为其代表一个 CPU 核心。每个 hart 都有自己的 PC、寄存器堆和 CSR 寄存器。有多个 hart 显然的优势是处理器能够并行处理多个任务，提高系统的吞吐量和响应速度。Hart 的状态管理由 OpenSBI 的 HSM（hart state management）扩展负责，其提供了对 hart 的启用、暂停、恢复等操作的支持。你可以通过阅读 SBI v2.0 规范来了解如何与 HSM 进行交互。

你可以尝试在 kernel 中实现多 hart 的调度与管理，进一步地，可以完善 scheduler 逻辑，支持将不同的进程分配到不同的 hart 上同时运行。
