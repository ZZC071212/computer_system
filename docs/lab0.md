...
!!! info "24.02.29 发布、24.03.21 截止验收与提交（三周）"
...
!!! warning
    ...
    对于自行编译 ip 并使用的同学，请在实验报告中进行注明（因为下发的 x86 ip 和自行编译的版本不同，可能会出现不一致行为），同时如果使用过程中遇到问题请及时联系助教。
...
!!! tip
    一般情况只需要同步 sys-3-project 即可，其他 submodule 均是用于自行编译工具链环境的（例如给出的 x86_64 的 ip 不能在自己的机器上使用）。

### 工程管理

为了方便同学们进行文件管理，也为了杜绝部分同学肆意修改提供的代码绕过测试点，我们在 sys2 的文件结构的基础上做了一些调整。

我们提供的代码挪到了 sys-3-project 仓库，同学们需要先 `git submodule update --init repo/sys-3-project` 得到该仓库，然后才可以使用里面的代码进行后续实验，当前文件结构如下：
...
sys-3-project 用于提供我们实现好的硬件代码，并且按照代码的功能进行了分门别类：一方面同学们写的代码不会和我们提供的代码混在一起，方便同学们定位自己需要的代码，以及将注意力集中在自己编写的代码上；另一方面也防止部分同学擅自修改代码，来绕过我们的测试点。

不同实验的代码都在这个仓库中，其中 lab0 的代码在分支 lab0 中，lab2 在分支 lab2 中，大家可以运行 `git checkout 分支名` 切换到对应分支。
...
- src/lab0 仅用于提供提供了需要补全的代码，或者需要使用的额外代码
...
详见 [sys2 lab6 文档](https://zju-sys.pages.zjusct.io/sys2/sys2-fa23/lab6/)。

## 实验要求与提交

对于在系统二中已经完成 lab6 验收的同学，本实验成绩直接计为满分，且不需要提交（请检查学在浙大上自己的分数）。但为了后续实验的方便，建议已经做完 lab6 的同学们也花一点时间来将自己的代码修改移植到当前框架中。不然在 lab2 的实验中有很大部分需要依赖于基于当前框架提供的代码，如果自行实现会有较大困难。

对于没有完成 lab6 的同学，本次实验中至少需要完成硬件异常处理以及仿真，并在实验课上进行验收。验收后将代码以及**简要**实验报告提交至学在浙大。

- 实验报告不要打包在代码中
- 代码只需要打包提交 project 文件夹（打包前 `make clean` 清除编译产物）
- 即最终需要在学在浙大上提交一份实验报告和一个代码压缩包两个文件