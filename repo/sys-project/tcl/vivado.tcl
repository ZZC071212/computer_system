...
set GENERAL_DIR $env(DIR_GENERAL)
set INCLUDE_DIR $env(DIR_INCLUDE)
set INCLUDE_CUSTOM $env(CUSTOM_INCLUDE)
...
if {[llength [glob -nocomplain [file join $GENERAL_DIR *.v]]] != 0} {
  read_verilog [glob [file join $GENERAL_DIR *.v]]
}
if {[llength [glob -nocomplain [file join $GENERAL_DIR *.sv]]] != 0} {
  read_verilog [glob [file join $GENERAL_DIR *.sv]]
}
if {[llength [glob -nocomplain [file join $INCLUDE_DIR *.vh]]] != 0} {
  read_verilog [glob [file join $INCLUDE_DIR *.vh]]
}
if {[llength [glob -nocomplain [file join $INCLUDE_CUSTOM *.vh]]] != 0} {
  read_verilog [glob [file join $INCLUDE_CUSTOM *.vh]]
}
